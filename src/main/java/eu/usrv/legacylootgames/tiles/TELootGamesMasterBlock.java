package eu.usrv.legacylootgames.tiles;
...
import static eu.usrv.legacylootgames.StructureGenerator.*;
...
public class TELootGamesMasterBlock extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            if (LootGamesLegacy.Rnd.nextInt(100) <= 10) {
                if (_mLastSoundTick < System.currentTimeMillis()) {
                    _mLastSoundTick = System.currentTimeMillis() + (LootGamesLegacy.Rnd.nextInt(90) + 30) * 1000;
                    worldObj.playSoundEffect(xCoord, yCoord, zCoord, String.format("%s:%s", LootGames.MODID, LGSounds.PUZZLE_MASTER_STRANGE), 0.5F, 1.0F);
                }
            }
        }
    }
    ...
    public void onBlockClickedByPlayer(Object object, EntityPlayer pPlayer) {
        try {
            ...
            if (!_mIsActive) {
                ...
                ILootGame tGame = LootGamesLegacy.GameMgr.selectRandomGame();
            ...
            LootGames.LOGGER.error(ex.toString());
        }
    }
}